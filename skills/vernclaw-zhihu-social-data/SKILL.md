---
name: vernclaw-zhihu-social-data
license: MIT
description: Use when calling TikHub-backed Zhihu social data operations through Vernclaw's CLI.
---

# Zhihu Social Data — CLI Skill

Call TikHub-backed Zhihu operations through Vernclaw Connect CLI and return normalized social data for AI agents.

## When to Use

- Run Zhihu research from an agent workflow without managing a TikHub key
- Fetch public search, profile, feed, detail, trend, or comment data supported by this connector
- Standardize Zhihu results into Vernclaw's compact JSON connector envelope
...
## Invocation
**Note:** The `social.tikhub.zhihu` connector is currently not available in the offline catalog. The following examples are preserved for reference when this connector becomes available.

```bash
# vernclaw-cli invoke social.tikhub.zhihu --operation hot_list --params '{}'
# vernclaw-cli describe social.tikhub.zhihu
```
...
| ------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| `--operation` | No       | TikHub operation name. Defaults to `hot_list` when omitted.                                                |
...
| ----------------------- | ---------------------------------- |
| `hot_list`              | Fetch Zhihu hot list rows.         |
| `column_article_detail` | Fetch Zhihu column article detail. |
...
## Workflow
**Note:** The `social.tikhub.zhihu` connector is currently not available in the offline catalog. The following workflow is preserved for reference when this connector becomes available.
...
- **Platform page**: <https://vernclaw.com/connectors/zhihu>
- **Connector detail**: <https://vernclaw.com/connectors/zhihu-social-data>
