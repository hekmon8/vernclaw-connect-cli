...
**Note:** The `search.tiktok` connector is currently not available in the offline catalog. The following examples are preserved for reference when this connector becomes available.

```bash
# vernclaw-cli invoke search.tiktok --query "AI tools" --limit 5
# vernclaw-cli invoke search.tiktok --query "AI tools" --hashtag aitools
# vernclaw-cli describe search.tiktok
```
...
**Note:** The `search.tiktok` connector is currently not available in the offline catalog. The following workflow is preserved for reference when this connector becomes available.
